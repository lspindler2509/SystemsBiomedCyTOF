...
run_kmeans2 <- function (data) 
{
  ...
  directory <- "systems_biomed/plots/sce/eval_raw/"
...
class_dir <- 'classification_matrix.csv'
types <- read_csv(class_dir)
print(types)
  
name <- "unscaled"
directory <- "systems_biomed/plots/sce/eval_results_raw/"
...
write.csv(binary.results, file = "results_sce_unscaled_original.csv", row.names = FALSE, col.names = FALSE)
