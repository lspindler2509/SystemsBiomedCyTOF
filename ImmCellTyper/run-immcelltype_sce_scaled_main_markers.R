...
run_kmeans2 <- function (data) 
{
  ...
  directory <- "systems_biomed/plots/sce_scaled/eval_raw_main_markers/"
...
name <- "scaled"
directory <- "systems_biomed/plots/sce_scaled/eval_results_raw_main_markers/"
...
exprs_matrix <- assay(sce, 'scaled_exprs')
exprs_matrix[exprs_matrix < 0] <- 0
assay(sce, 'scaled_exprs') <- exprs_matrix
exprs_matrix <- NULL

exprs <- t(assay(sce, 'scaled_exprs'))
binary.results <- binaryClass2(exprs, class_dir)
print(class(binary.results))
write.csv(binary.results, file = "results_sce_scaled_main.csv", row.names = FALSE, col.names = FALSE)
